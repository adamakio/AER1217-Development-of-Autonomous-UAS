...
def plot_trajectory(t_scaled,
                    ...
                    ref_ax, ref_ay, ref_az,    # (unused but kept for signature)
                    gates,
                    obstacles,
                    vel_stride=10,             # plot one vel vector every N samples
                    vel_scale=0.4):            # arrow length scaling
    """
    Visualise reference trajectory, gates and obstacles.
    Adds velocity arrows (quiver) every `vel_stride` samples.
    """
    # ------------------------------------------------------------------ 3‑D figure
    fig = plt.figure()
    ...
    ax.plot3D(ref_x, ref_y, ref_z, color="blue", label="Trajectory")
    ax.scatter3D(waypoints[:, 0], waypoints[:, 1], waypoints[:, 2],
                 color="blue", s=15, label="Waypoints")

    # Start & goal markers
    ax.scatter3D(-1.0, -3.0, 1.0, color="black",  s=50, label="Start")
    ax.scatter3D(-0.5,  2.0, 1.0, color="orange", s=50, label="Goal")

    # ------------------------------------------------------------- Velocity vectors
    ### NEW : magnitude‑scaled arrows ###
    idx = np.arange(0, len(t_scaled), vel_stride)            # subsample

    # stack and compute norms
    vel_stack = np.vstack((ref_vx[idx], ref_vy[idx], ref_vz[idx])).T   # shape (M,3)
    speed_norm = np.linalg.norm(vel_stack, axis=1)                     # |v| for each arrow
    max_norm   = speed_norm.max() if speed_norm.size else 1.0          # avoid div/0

    # scale factor so the *longest* arrow has length = vel_scale metres
    scale = vel_scale / max_norm

    ax.quiver(ref_x[idx], ref_y[idx], ref_z[idx],            # tail positions
            ref_vx[idx] * scale,
            ref_vy[idx] * scale,
            ref_vz[idx] * scale,
            normalize=False,                               # keep proportional lengths
            color="purple", linewidth=0.8, label="Velocity")

    # -------------------------------------------------------------------- Gates
    for gate in gates:
        x, y, z, _, _, yaw, _ = gate
        gate_w = 0.4; gate_h = 0.4; center_z = 1.0
        dx = (gate_w / 2) * np.cos(yaw)
        dy = (gate_w / 2) * np.sin(yaw)
        corners = np.array([
            [x - dx, y - dy, center_z - gate_h / 2],
            [x + dx, y + dy, center_z - gate_h / 2],
            [x + dx, y + dy, center_z + gate_h / 2],
            [x - dx, y - dy, center_z + gate_h / 2],
        ])
        ax.add_collection3d(Poly3DCollection([corners], color="green", alpha=0.3))

    # ------------------------------------------------------------------ Obstacles
    u, v = np.mgrid[0:2*np.pi:15j, 0:np.pi:10j]
    for obs in obstacles:
        ox, oy = obs[:2]; zc = 1.0
        r_nom, r_unc = 0.06, 0.2
        xs = ox + r_nom * np.cos(u) * np.sin(v)
        ys = oy + r_nom * np.sin(u) * np.sin(v)
        zs = zc + r_nom * np.cos(v)
        ax.plot_surface(xs, ys, zs, color="red", alpha=0.6)          # actual
        xs_u = ox + r_unc * np.cos(u) * np.sin(v)
        ys_u = oy + r_unc * np.sin(u) * np.sin(v)
        zs_u = zc + r_unc * np.cos(v)
        ax.plot_surface(xs_u, ys_u, zs_u, color="red", alpha=0.2)    # uncertainty

    # ---------------------------------------------------------------- Formatting
    ax.set_title("3‑D Trajectory with Velocity Vectors")
    ax.set_xlim([-3.5, 3.5]); ax.set_xlabel("x (m)")
    ax.set_ylim([-3.5, 3.5]); ax.set_ylabel("y (m)")
    ax.set_zlim([0.0, 2.0]);   ax.set_zlabel("z (m)")
